...
// Get teacher case details with available columns
$stmt = $pdo->prepare("
    SELECT c.*, u.first_name, u.last_name, u.email, u.phone
    FROM support_cases c 
    JOIN users u ON c.teacher_id = u.id 
    WHERE c.id = :id AND c.logged_by_role = 'teacher'
");
...
$stmt = $pdo->prepare("
    SELECT cr.*, 
           CASE 
               ...
               WHEN cr.user_type = 'Teacher' THEN CONCAT(u.first_name, ' ', u.last_name)
               ELSE cr.user_type
           END as user_display_name
    FROM case_responses cr
    LEFT JOIN users u ON cr.user_type = 'Teacher' AND cr.user_id = u.id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: admin_view_teacher_case.php?id=" . $case_id);
    exit;
}
?>
...
<head>
    ...
    <title>Teacher Case <?= $case['case_number'] ?> - Admin - NovaTech</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        .teacher-message { background-color: #e3f2fd; border-left: 4px solid #2196f3; }
        ...
        <div class="row">
            <div class="col-md-10 mx-auto">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1>Teacher Case: <?= $case['case_number'] ?></h1>
                        <p class="text-muted">Logged by Teacher</p>
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="case-info mb-3">
                                    ...
                                    <p class="mb-0"><strong>Logged by:</strong> Teacher</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="message-container mb-3">
                            ...
                            <?php else: ?>
                                <?php foreach ($responses as $response): ?>
                                    <div class="p-3 mb-2 rounded <?= $response['user_type'] === 'Teacher' ? 'teacher-message' : ($response['user_type'] === 'Admin' ? 'admin-message' : 'system-message') ?>">
                                        <div class="d-flex justify-content-between">
                                            <strong>
                                                <?= $response['user_display_name'] ?>
                                                <?php if ($response['user_type'] === 'Teacher'): ?>
                                                    (Teacher)
                                                <?php endif; ?>
                                            </strong>
                            ...
                            <form method="POST">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" name="message" rows="3" 
                                              placeholder="Type your response to the teacher..." required></textarea>
                                </div>
