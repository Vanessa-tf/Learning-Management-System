...
// Get case details with parent/student information - FIXED QUERY
$stmt = $pdo->prepare("
    SELECT c.*, s.first_name, s.surname, s.email as student_email, s.phone as student_phone,
           s.financier_name, s.financier_email, s.financier_phone, s.financier_relationship as relationship
    FROM support_cases c 
    JOIN students s ON c.student_id = s.id 
    WHERE c.id = :id AND c.logged_by_role = 'parent'
");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: admin_view_parent_case.php?id=" . $case_id);
    exit;
}
?>
...
<head>
    ...
    <title>Parent Case <?= $case['case_number'] ?> - Admin - NovaTech</title>
        ...
        <div class="row">
            <div class="col-md-10 mx-auto">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1>Parent Case: <?= $case['case_number'] ?></h1>
                        <p class="text-muted">Logged by Parent/Guardian</p>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="case-info mb-3">
                                    ...
                                    <p class="mb-0"><strong>Phone:</strong> <?= htmlspecialchars($case['student_phone']) ?></p>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="case-info mb-3">
                                    <h6>Parent/Guardian Information</h6>
                                    <?php if (!empty($case['financier_name'])): ?>
                                        <p class="mb-1"><strong>Name:</strong> <?= htmlspecialchars($case['financier_name']) ?></p>
                                        <p class="mb-1"><strong>Relationship:</strong> <?= htmlspecialchars($case['relationship']) ?></p>
                                        <p class="mb-1"><strong>Email:</strong> <?= htmlspecialchars($case['financier_email']) ?></p>
                                        <p class="mb-0"><strong>Phone:</strong> <?= htmlspecialchars($case['financier_phone']) ?></p>
                                    <?php else: ?>
                                        <p class="mb-0 text-muted">No parent/guardian information available</p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="p-3 mb-2 rounded <?= $response['user_type'] === 'Student' ? 'student-message' : ($response['user_type'] === 'Admin' ? 'admin-message' : 'system-message') ?>">
                                        <div class="d-flex justify-content-between">
                                            <strong>
                                                ...
                                                <?php if ($response['user_type'] === 'Student'): ?>
                                                    (Parent)
                                                <?php endif; ?>
                                            </strong>
                            ...
                            <form method="POST">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" name="message" rows="3" 
                                              placeholder="Type your response to the parent..." required></textarea>
                                </div>
